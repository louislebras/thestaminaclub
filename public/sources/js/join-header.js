...
var limit2 = 0.6, //implies 40 vh or 40% of viewport height
  scrollLimit = 0;

$(window)
  .resize(function () {
    scrollLimit = $(window).height() * limit2;
  })
  .scroll(function () {
    var scroll = $(window).scrollTop();

    if (scroll >= scrollLimit) {
      $(".logo-fixed-join").addClass("hideLogo");
    } else {
      $(".logo-fixed-join").removeClass("hideLogo");
    }
  })
  .trigger("resize")
  .trigger("scroll"); // trigger both events on start to force initial setup
